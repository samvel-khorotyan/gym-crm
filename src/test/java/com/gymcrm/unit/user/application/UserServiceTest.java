...
// CreateUserCommand command = new CreateUserCommand("John", "Doe",
// UserType.TRAINER);
// String generatedUsername = "john.doe";
// String generatedPassword = "securePassword123";
// User user = new User(UUID.randomUUID(), "John", "Doe", generatedUsername,
// generatedPassword,
// true,
// UserType.TRAINER);
//
// MockedStatic<UserUtil> mockedUtil = mockStatic(UserUtil.class);
// mockedUtil.when(() -> UserUtil.getBaseUsername(command.getFirstName(),
// command.getLastName()))
// .thenReturn(generatedUsername);
// mockedUtil.when(() -> UserUtil.generateUniqueUsername(anySet(),
// eq(generatedUsername)))
// .thenReturn(generatedUsername);
...
// CreateUserCommand command = new CreateUserCommand(null, "Doe",
// UserType.TRAINER);
//
// RuntimeException exception = assertThrows(RuntimeException.class, () ->
// userService.create(command));
// assertInstanceOf(IllegalArgumentException.class, exception.getCause());
// assertEquals("First name and last name cannot be null or empty",
...
// CreateUserCommand command = new CreateUserCommand("John", " ",
// UserType.TRAINER);
//
// RuntimeException exception = assertThrows(RuntimeException.class, () ->
// userService.create(command));
...
// User mockUser = new User(UUID.randomUUID(), "John", "Doe", username,
// "password123", true, null);
...
// @Test
// void
// loadUserByUsername_ShouldThrowUserNotFoundException_WhenUserDoesNotExist() {
// String username = "non-existent-user";
// when(loadUserPort.findByUsername(username)).thenThrow(new
...
// @Test
// void
// loadUserByUsername_ShouldThrowRuntimeException_WhenUnexpectedErrorOccurs() {
// String username = "john.doe";
// when(loadUserPort.findByUsername(username)).thenThrow(new
...
// User user1 = new User(UUID.randomUUID(), "John", "Doe", "john.doe",
// "password", true,
// UserType.TRAINER);
// User user2 = new User(UUID.randomUUID(), "Jane", "Smith", "jane.smith",
// "password", true,
// UserType.TRAINEE);
...
// when(loadUserPort.findAll()).thenThrow(new RuntimeException("Database
// error"));
//
// RuntimeException exception = assertThrows(RuntimeException.class, () ->
// userService.loadAll());
...
// UpdatePasswordCommand command = new UpdatePasswordCommand("john.doe",
// "oldPassword",
// "newPassword");
// User user = new User(UUID.randomUUID(), "John", "Doe", "john.doe",
// "oldPassword", true,
// UserType.TRAINER);
// when(loadUserPort.findByUsername(command.getUsername())).thenReturn(user);
// doNothing().when(authenticationUseCase).authenticate(command.getUsername(),
...
// verify(authenticationUseCase, times(1)).authenticate(command.getUsername(),
// command.getOldPassword());
...
// @Test
// void
// updatePassword_ShouldThrowUnauthorizedException_WhenAuthenticationFails() {
// UpdatePasswordCommand command = new UpdatePasswordCommand("john.doe",
// "wrongPassword",
// "newPassword");
// doThrow(new UnauthorizedException("Invalid
// credentials")).when(authenticationUseCase)
...
// UpdatePasswordCommand("non-existent-user", "oldPassword",
// "newPassword");
// when(loadUserPort.findByUsername(command.getUsername())).thenThrow(new
...
// error")).when(updateUserPort).save(user);
//
// RuntimeException exception = assertThrows(RuntimeException.class, () ->
// userService.updatePassword(command));
// assertEquals("Unexpected error occurred. Please contact support.",
